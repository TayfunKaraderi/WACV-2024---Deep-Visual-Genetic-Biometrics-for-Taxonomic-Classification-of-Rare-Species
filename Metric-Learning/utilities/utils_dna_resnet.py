...
from models.TripletResnetSoftmax import DNA_FCL_
...
class Utilities:
    ...
    def setupForTraining(self, args):
        ...
        data_loader = data.DataLoader(dataset, batch_size=args.batch_size, num_workers=2, shuffle=True)
        ...
        loss_fn_2 = OnlineReciprocalSoftmaxLoss(triplet_selector) #TripletLoss(triplet_selector)
        ...
        if "Reciprocal" in args.loss_function:
            ...
            optimiser_2 = optim.SGD(model_2.parameters(), lr=args.learning_rate)#, weight_decay=args.weight_decay)

        else:
            optimiser = optim.SGD(model.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=args.weight_decay)
            optimiser_2 = optim.SGD(model_2.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=args.weight_decay)


        return data_loader, model, model_2, loss_fn, loss_fn_2, optimiser, optimiser_2
    ...
    # Log information 
    def logTrainInfo(self, epoch, step, loss_mean, loss_triplet=None, loss_triplet_2=None, loss_softmax=None):
        # Add to our arrays
        ...
        if loss_triplet_2 != None: log_message += f", loss_triplet_FCL: {loss_triplet_2:.5f}"
    ...
    def test(self, step):
        ...
        #accuracy_dna = str(accuracy_.decode('utf-8').split("Accuracy_dna=")[1])
        
        self.accuracies.append(accuracy)

        # Report the accuracy
        print(f"Accuracy: {accuracy}%")
        #print(f"Accuracy_dna: {accuracy_dna}%")
