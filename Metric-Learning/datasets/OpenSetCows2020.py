...
from torchvision.transforms import Compose, ToTensor, Normalize, RandomRotation, CenterCrop,GaussianBlur, RandomResizedCrop, ToPILImage
import torchvision.transforms as transforms
...
def find_threshold(confidence_scores, split_ratio1, split_ratio2):
    
    length = len(confidence_scores)
    
    split_index1 = int(length*split_ratio1)
    split_index2 = int(length*split_ratio2)
    
    
    cs = np.sort(confidence_scores)
    
    threshold_score1 = cs[split_index1]
    threshold_score2 = cs[split_index2]

    
    indexes1 = np.where(confidence_scores <= threshold_score1) # low confidence
    indexes2 = np.where(confidence_scores > threshold_score2) # high confidence

    return threshold_score1, threshold_score2, indexes1[0], indexes2[0]

# import confidense scores as numpy array
confidences = np.load('/content/drive/MyDrive/foram-MetricLearningIdentification-master/datasets/OpenSetCows2020/confidence.npy')

thresold1, thresold2, indexes1, indexes2  = find_threshold(confidences, 0.03, 0.97)
indexess1 = indexes1[1:-1]-1
indexess2 = indexes2[1:-1]-1

a = np.where(confidences <11656)
a2 = np.where(confidences >1495)
indexess3 = np.intersect1d(a[0], a2[0]) -1


indexess4 = np.concatenate((indexess1, indexess3), axis=0)
indexess5 = np.concatenate((indexess2, indexess3), axis=0)

random_index = np.array((list(range(0,27730,1))))

random_list = np.array(random.sample(range(0, 27390), 250))
...
RandomResizedCrop(size=224, scale=(0.9, 1.0), ratio=(1, 1)) #
])
...
class OpenSetCows2020(data.Dataset):
	...
	def __init__(	self,
		...
		self.__train_images_dir = os.path.join(self.__root, "images/train") ##--- cd colab. ***
	...
	def __getitem__(self, index):
		...
		if self.__transform:
			...
			img_anchor = train_transform(img_anchor)
			img_anchor = gray_transform(img_anchor)
			img_pos = train_transform(img_pos)
			img_pos = gray_transform(img_pos)
			img_neg = train_transform(img_neg)
			img_neg = gray_transform(img_neg)
