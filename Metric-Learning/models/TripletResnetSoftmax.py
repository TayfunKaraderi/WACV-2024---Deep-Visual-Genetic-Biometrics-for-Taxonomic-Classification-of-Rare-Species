...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.relu = nn.ReLU(inplace=True) # changed true to false
...
#transfer learning 
'''
def TripletResnet50Softmax(pretrained=False,  num_classes=32, embedding_size=256, **kwargs):
    ...
    if pretrained:
        ...
        model = Triplet_ResNet_Softmax(Bottleneck, [3, 4, 6, 3], num_classes=32, **kwargs)
        #freeze weights
        for param in model.parameters():
            param.requires_grad = False
        model.fc = nn.Linear(2048, 1000)
        model.fc_embedding = nn.Linear(1000, 256)
        model.fc_softmax = nn.Linear(1000, 32) #num_classes
        #PATH =  "/content/drive/MyDrive/foram-MetricLearningIdentification-master/output/_____A_DNA_Visual_train/resent_im_pretrained_acc70per/best_model_state.pkl"
        #PATH =  "/content/drive/MyDrive/foram-MetricLearningIdentification-master/output/gray_scale_train_13ep_88acc/current_model_state.pkl"
        PATH = "/content/drive/MyDrive/foram-MetricLearningIdentification-master/output/LTR_Control_no_WD/best_model_state.pkl" #ALL_AUG_ep40/best_model_state.pkl
    ...
    #model.fc_softmax = nn.Linear(1000, 32)
...
def TripletResnet50Softmax(pretrained=False,  num_classes=32, embedding_size=128, **kwargs):
    model = Triplet_ResNet_Softmax(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, **kwargs)
...
class DNA_FCL(nn.Module):
    def __init__(self):
        super(DNA_FCL, self).__init__()
        
        self.linear_relu_stack = nn.Sequential(
            nn.Linear(256, 256),#.weight.data.copy_(torch.eye(256)),
            nn.ReLU(inplace=True),
            nn.Linear(256, 128),#.weight.data.copy_(torch.eye(256)),
            nn.ReLU(inplace=True),
            )
        ...
        self.dense.weight.data.copy_(torch.eye(256)) # add noise
...
def DNA_FCL_():
    model = DNA_FCL()
    #transfer learning
    #PATH =  "/content/drive/MyDrive/foram-MetricLearningIdentification-master/output/fold0_0/current_fcl_model_state.pkl"
    #checkpoint = torch.load(PATH)
    #model.load_state_dict(checkpoint['model_state']) #
    model.train()
